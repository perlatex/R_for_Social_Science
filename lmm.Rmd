...
complete_pooling <-
  ...
  rename(Intercept = `(Intercept)`, slope = experience) %>%
  mutate(pooled = "complete_pool") %>%
  select(pooled, Intercept, slope)
...
partial_pooling <-
  coef(m4)$department %>%
  rownames_to_column() %>%
  rename(level = rowname, Intercept = `(Intercept)`, slope = experience) %>%
  mutate(pooled = "partial_pool") %>%
  select(pooled, level, Intercept, slope)
...
```{r}
un_pooling <- no_pool %>%
  select(department, term, estimate) %>%
  pivot_wider(
    names_from = term,
    values_from = estimate
  ) %>%
  ...
  select(pooled, level = department, Intercept, slope)
